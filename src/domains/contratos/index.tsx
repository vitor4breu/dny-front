...
import { Add, AttachMoney, Edit, FilterAlt, FilterAltOutlined, Info, Money } from '@mui/icons-material';
...
const Contratos: React.FC = () => {
  ...
  const [paginacao, setPaginacao] = useState<Paginacao>({ pagina: 0, linhasPorPagina: 10 });
  ...
  const { contratos, totalContratos, loading, error } = useAppSelector((state) => state.contract);
  ...
  return (
    <Container>
      ...
      <IconButton onClick={e => setAnchorEl(e.currentTarget)} className="filter-icon-buttom">
      ...
      <div>
        ...
        <TableContainer>
          ...
          <TablePagination
            ...
            onPageChange={(_, paginaAtual) => setPaginacao({...paginacao, pagina: paginaAtual})}
            rowsPerPage={paginacao.linhasPorPagina}
            onRowsPerPageChange={(event) => setPaginacao({...paginacao, pagina: parseInt(event.target.value, 10)})}
