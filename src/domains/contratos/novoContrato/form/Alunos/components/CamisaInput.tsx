...
import { UNIFORM_GENDER, UNIFORM_SIZES, TSHIRT_COLORS } from '@utils/consts';
...
const CamisaInput: React.FC<CamisaInputProps> = ({ aluno, index }) => {
  ...
  const [nomePersonalizado, setNomePersonalizado] = useState<boolean>(!!aluno.camisa?.nomePersonalizado);
    ...
    <>
      <Grid item md={12} pt="4px !important">
        <MuiBox display="flex" justifyContent="space-between">
          <Typography variant="subtitle1" fontWeight={"bold"} display={'inline'}> Camisa</Typography>
      ...
      <Grid item xs={12} md>
        <FormControl fullWidth>
          ...
          <Select
            ...
            onChange={(e) => updateAluno(index, 'camisa', { ...aluno.camisa, modelagem: e.target.value })}
            displayEmpty
          >
      ...
      <Grid item xs={12} md={!detalhesPedido.possuiNomePersonalizado ? 3 : 2}>
        <FormControl fullWidth>
          ...
          <Select
            ...
            onChange={(e) => updateAluno(index, 'camisa', { ...aluno.camisa, tamanho: e.target.value })}
            displayEmpty
          >
            <MenuItem value="" disabled>
        ...
        <Grid item xs={12}>
          <FormControl fullWidth>
            ...
            <TextField
              ...
              onChange={(e) => updateAluno(index, 'camisa', { ...aluno.camisa, nomePersonalizado: e.target.value })}
            />
          </FormControl>
        </Grid>
      )}
