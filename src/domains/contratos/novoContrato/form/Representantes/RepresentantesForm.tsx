...
import DeliveryForm from "../Envio/EnvioForm";
...
const RepresentanteForm: React.FC = () => {
  ...
  const representantes = useFormStore(
    (state) => state.representantes
  ) as Array<Representante>;
  const setRepresentantes = useFormStore((state) => state.setRepresentantes);
  const handleRemoveRepresentante = useFormStore(
    (state) => state.handleRemoveRepresentante
  );
  const handleAddRepresentante = useFormStore(
    (state) => state.handleAddRepresentante
  );
          ...
          <Grid item xs={12}>
            <FormControl fullWidth>
              ...
              <TextField
                ...
                onChange={(e) =>
                  setEscolaInput({ escola: e.target.value, error: undefined })
                }
