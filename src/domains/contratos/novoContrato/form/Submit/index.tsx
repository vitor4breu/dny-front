...
const SubmitForm: React.FC = () => {
  ...
  const displayValidationErrors = (errors: Array<Partial<Record<keyof Aluno, string>>>) => {
      ...
      toast.error(
        <div>
          ...
          {errorMessages.map(({ field, message }, index) => (
            <div key={`${field}-${index}`}>• {field}: {message}</div>
          ))}
        </div>,
  ...
  const handleSubmit = async () => {
    ...
    if(!detalhesPedido.possuiCaneca)
        alunosRequest = alunosRequest.map(aluno => ({...aluno, caneca: undefined}));

    if(!detalhesPedido.possuiMoletom)
        alunosRequest = alunosRequest.map(aluno => ({...aluno, moletom: undefined}));

    if(!detalhesPedido.possuiCamiseta)
        alunosRequest = alunosRequest.map(aluno => ({...aluno, camisa: undefined}));
    
    if(!detalhesPedido.possuiTirante)
        alunosRequest = alunosRequest.map(aluno => ({...aluno, caneca: {...aluno.caneca, tirante: false}}));
