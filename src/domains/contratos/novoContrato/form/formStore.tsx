import { stat } from 'fs';
import { Aluno, Cor, DetalhesEnvio, DetalhesPedido, Endereco, Representante, TipoEnvio } from '../types';
...
const useFormStore = create<FormState>((set) => ({
  ...
  handleAddRepresentante: () => set((state) => ({ representantes: [...state.representantes, { nome: '', sobrenome: '', telefone: '', principal: false } as Representante]})),
  handleRemoveRepresentante: (index) => set((state) => ({ representantes: state.representantes.filter((_, i) => i !== index)})),
  ...
  updateEndereco: (field, value) => set((state) => ({
    detalhesEnvio: {
      ...state.detalhesEnvio,
      endereco: {
  ...
  handleAddAluno: () => set((state) => ({ alunos: [...state.alunos, obterDefaultAluno(state.detalhesPedido)] })),
  handleRemoveAluno: (index) => set((state) => ({ alunos: state.alunos.filter((_, i) => i !== index) })),
  updateAluno: (index, field, value) => set((state) => ({
    alunos: state.alunos.map((aluno, i) => i === index ? { ...aluno, [field]: value } : aluno)
  })),
  coresMoletom: [],
